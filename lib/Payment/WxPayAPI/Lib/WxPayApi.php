<?php
namespace Lib\Payment\WxPayAPI\Lib;
...
class WxPayApi {
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj, $timeOut = 6, $config = [])
	{
	    if (empty($config)) {
            throw new WxPayException("请配置微信商户信息!");
        }
...
//		}
		...
		$inputObj->SetAppid($config['wechat_appid']);//公众账号ID
		$inputObj->SetMch_id($config['wechat_mchid']);//商户号
		...
		// 签名
		$inputObj->SetSign($config['wechat_key']);
		$xml = $inputObj->ToXml();
		$response = self::postXmlCurl($xml, $url, false, $timeOut);
		$result = WxPayResults::Init($response, $config['wechat_key']);
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function orderQuery($inputObj, $config = [], $timeOut = 6)
	{
		...
		$inputObj->SetMch_id($config['wechat_mchid']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		$inputObj->SetSign($config['wechat_key']);//签名
		$xml = $inputObj->ToXml();
		$response = self::postXmlCurl($xml, $url, false, $timeOut);
		$result = WxPayResults::Init($response, $config['wechat_key']);
        //$startTimeStamp = self::getMillisecond();//请求开始时间
	...
	public static function shorturl($inputObj, $timeOut = 6)
	{
 	 ...
 	 */
	public static function notify($callback, $appid, $store_id, $xml, $key, &$msg)
	{
		//如果返回成功则验证签名
		try {
